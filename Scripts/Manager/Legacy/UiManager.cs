...
namespace legacy
{
    public class UiManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                StartCoroutine(ResponseAPIStringAsync("SELECT", @"INSERT INTO TB_ALARM_DATA
            ...
            {
                StartCoroutine(ResponseAPIStringAsync("SELECT", @"SELECT TOP 1 *
                FROM TB_ALARM_DATA
                ORDER BY ALADT DESC;
            ", str => Debug.Log(str)));
            }
            ...
            {
                StartCoroutine(ResponseAPIStringAsync("SELECT", @"DELETE FROM TB_ALARM_DATA
                WHERE ALAIDX = (
                    SELECT TOP 1 ALAIDX
                    FROM TB_ALARM_DATA
                    ORDER BY ALADT DESC
            ...
            {
                StartCoroutine(ResponseAPIStringAsync("SELECT", @"
            ...
            {
                StartCoroutine(ResponseAPIStringAsync("SELECT", @"IF OBJECT_ID('GET_UPDATED_ALARM_LOG', 'P') IS NOT NULL
    PRINT 'GET_UPDATED_ALARM_LOG 프로시저가 존재합니다.';
        ...
        IEnumerator ResponseAPIStringAsync(string type, string query, System.Action<string> callback)
        {
            var data = new
            {
                SQLType = type,
