...
namespace legacy
{
    public class DbManager : MonoBehaviour
    {
        ...
        IEnumerator GetObssFunc(Action<List<ObsData>> callback)
        {
            ...
            yield return StartCoroutine(ResponseAPIStringAsync(QueryType.SELECT.ToString(), query, (response) =>
            {
        ...
        IEnumerator GetToxinValueLastFunc(int obsId, Action<List<CurrentDataModel>> callback)
        {
            ...
            yield return StartCoroutine(ResponseAPIStringAsync(QueryType.SELECT.ToString(), query, (response) =>
            {
                Debug.Log("GetToxinValueLastFunc HNS Toxin Value Response: " + response);
            ...
            !(current.boardidx == 3 && current.hnsidx > 7)     // 수질 센서 6개로 제한
        ).ToList();
            callback(curDatas);
        }
        IEnumerator GetToxinDataFunc(int obsId, Action<List<ToxinData>> callback)
        {
            ...
            yield return StartCoroutine(ResponseAPIStringAsync(QueryType.SELECT.ToString(), query, (response) =>
            {
                Debug.Log("GetToxinDataFunc API HNS Response: " + response);
            ...
            toxinDatas = toxinDatas.Where(toxin =>
                ...
                !(toxin.boardid == 3 && toxin.hnsid > 7)     // 수질 센서 6개로 제한
            ).ToList();
