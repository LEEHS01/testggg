...
namespace Assets.Scripts.Manager
{
    public class ModelManager : MonoBehaviour, ModelProvider
    {
        ...
        private void Start()
        {
            ...
            dbManager.GetGroups(groups => groups.ForEach(group => this.groups.Add(group)));
            dbManager.GetBoardSpecs(boardSpecs => boardSpecs.ForEach(spec => this.brdSpecs.Add(spec)));
            dbManager.GetAlarmsActivated(alarms => alarms.ForEach(alarm => this.alarmsActivated.Add(alarm)));
            dbManager.GetAlarms(alarms => alarms.ForEach(alarm => this.alarmsWhole.Add(alarm)));
            dbManager.GetConf(conf => this.conf = conf);
            ...
            uiManager.Register(UiEventType.SelectObs, OnSelectObs);
            uiManager.Register(UiEventType.RequestResetInspect, OnRequestResetInspect);
            uiManager.Register(UiEventType.RequestResetThreshold, OnRequestResetThreshold);
            ...
            UiManager.Instance.Register(UiEventType.RequestHistoryTimeSeries, OnRequestHistoryTimeSeries);
        ...
        private void AwaitInitiating()
        {
            ...
            if (initTryCount > 5)
            {
                Debug.Log($"obss {obss.Count} groups {groups.Count} DB URL : {DbManager.db_url}");
        ...
        //1초 주기
        void GetRecentDataChangedProcess() 
        {
            dbManager.GetObss(obss =>
            {
                this.obss.Clear();
                obss.ForEach(obs => this.obss.Add(obs));

                uiManager.Invoke(UiEventType.ObsUpdate);
            });


            dbManager.GetMeasTableRange(GetCurrentObsIdx(), DateTime.Now.AddHours(-1), DateTime.Now,
                res => { 
                    trendRealtime = res;
                    uiManager.Invoke(UiEventType.UpdateRealtimeTimeSeries);
                },
                e => 
                    Debug.LogError($"ModelManager - GetMeasTableRange : {e.Message}")
                );



            // 1초 후 재실행
            DOVirtual.DelayedCall(1, GetRecentDataChangedProcess);
        }

        private void OnInitiate(object obj)
        {
            GetAlarmChangedProcess();
            GetRecentDataChangedProcess();
        }
        ...
        {
            if (obj is int obsIdx) 
            {
                this.currentObsId = obsIdx;
            }
        }
        //    private void OnInitiate(object obj)
        //    {
        ...
        //                        else if (toxin.serious > 0 && lastValue >= toxin.serious) // 경계 임계값
        ...
        private void OnRequestResetInspect(object obj)
        {
            if(obj is not int obsIdx) return;
            dbManager.RequestResetInspect(obsIdx, s =>
            {
                Debug.Log($"ResetInspect - 관측소 {obsIdx}의 점검 정보가 초기화되었습니다.");
            },
            e => 
            {
                Debug.LogError($"ResetInspect - 관측소 {obsIdx}의 점검 정보 초기화에 실패했습니다. 오류: {e}");

            });

        }

        private void OnRequestResetThreshold(object obj)
        {
            if (obj is not int obsIdx) return;
            dbManager.RequestResetThreshold(obsIdx, s =>
            {
                Debug.Log($"ResetInspect - 관측소 {obsIdx}의 임계값 초기화되었습니다.");
            },
            e =>
            {
                Debug.LogError($"ResetInspect - 관측소 {obsIdx}의 임계값 초기화에 실패했습니다. 오류: {e}");

            });
        }

        private void OnRequestHistoryTimeSeries(object obj)
        {
            if (obj is not (int groupIdx, int obsIdx, int sensorIdx, DateTime from, DateTime to)) return;

            dbManager.GetMeasTableRange(obsIdx, from, to, 
                res => 
                {
                    trendHistory = res.table.Select(val => {
                        var valset = val.valSet.Find(valList => valList.idx == sensorIdx);
                        return (val.timestamp, (valset.val, valset.isMissing));
                    }).ToList();


                    uiManager.Invoke(UiEventType.UpdateHistoryTimeSeries);
                }
        ...
        List<BoardSpecInfo> brdSpecs = new();
        List<GroupInfo> groups = new();

        //Dynamic Data
        List<ObservatoryInfo> obss = new();
        List<AlarmInfo> alarmsActivated = new();
        List<AlarmInfo> alarmsWhole = new();
        TimeSeriesInfo trendRealtime = null;//currentObs에 의존
        ...
        public int GetCurrentObsIdx() => currentObsId;


        public List<ObservatoryInfo> GetObss() => new List<ObservatoryInfo>(obss);


        public ObservatoryInfo GetObsByIdx(int obsIdx) => obss.Find(obs => obs.obsIdx == obsIdx);


        public List<AlarmInfo> GetAlarmsWhole() => new List<AlarmInfo>(alarmsWhole);


        public List<AlarmInfo> GetAlarmsActivated() => new List<AlarmInfo>(alarmsActivated);


        public List<GroupInfo> GetGroups() => new List<GroupInfo>(groups);

        public GroupInfo GetGroupByIdx(int groupIdx) => groups.Find(grp => grp.groupIdx == groupIdx);


        public List<BoardSpecInfo> GetBoardSpecs() => new List<BoardSpecInfo>(brdSpecs);
